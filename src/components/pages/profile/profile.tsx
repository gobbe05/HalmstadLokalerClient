...
const ProfileCard = ({openProfile, setOpenProfile, id}: {openProfile: boolean, setOpenProfile: React.Dispatch<React.SetStateAction<boolean>>, id: string}) => {
  ...
  return (
    <div onClick={(event) => {
    ...
    }} className={`w-screen h-screen fixed left-0 top-0 bg-gray-500 bg-opacity-50 ${!openProfile && "hidden" }`}>
        <div ref={boxRef} className="bg-white rounded-2xl shadow-lg max-w-2xl overflow-hidden w-full p-8 mx-auto mt-10">
            ...
            <div className="flex flex-col items-center">
                ...
                <h2 className="text-2xl font-bold text-gray-800">{profileData && profileData.user.username}</h2>
                <p className="text-sm text-gray-500 mb-10">{profileData && profileData.user.email}</p>
                ...
                <div className="w-full overflow-scroll max-h-[40dvh]">
                    ...
                    <div className="flex justify-center mt-4">
                        <button onClick={() => {setLimit(prev => prev == null ? 3 : null)}} className="text-gray-700 border border-gray-700 hover:bg-gray-700 hover:text-white rounded-full px-4 py-2 transition-all">{limit == null ? "Visa mindre" : "Visa alla dina annonser"}</button>
                    </div>
